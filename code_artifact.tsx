...
function EgymodApp() {
  ...
  // دالة حذف وتصفية حساب الشريك بالكامل
  function settleAndRemovePartner(partnerId) {
    ...
    notify(`تمت تصفية حساب الشريك ${p.name} وحذفه من رأس مال الشركة بنجاح!`);
  }
...
/* ============================================================
   1. شاشة "الخزينة والحسابات" الرئيسية (مطابقة للصورة 1)
   ============================================================ */
function TreasuryMainScreen({ partners, expenses, salaryLog, totals, onNavigate, onBack }) {
  ...
  const treasuryButtons = [
    ...
    { key: "treasuryDistribute", label: "توزيع الأرباح", icon: Wallet, tone: "roseDark" },
    { key: "treasuryEmployees", label: "إضافة موظف ورواتب وسلف الموظفين", icon: UserCog, tone: "dark" },
    { key: "treasuryExpenses", label: "المصروفات", icon: CreditCard, tone: "copper" },
  ];
  ...
  return (
    <div style={styles.container}>
      ...
      <section style={styles.grid}>
        {treasuryButtons.map((b) => (
          <DashButton key={b.key} label={b.label} Icon={b.icon} tone={b.tone} onClick={() => onNavigate(b.key)} />
        ))}
      </section>
...
/* ============================================================
   2. شاشة الشركاء ورأس المال (مطابقة للصورة 2 مع تصفية الشريك)
   ============================================================ */
function PartnersScreen({ partners, onAddPartner, withdrawalsLog, setWithdrawalsLog, onSettlePartner, onBack, notify }) {
  ...
  return (
    <div style={styles.container}>
              ...
              {partnersCalculated.map((p) => (
                <tr key={p.id} style={{ borderBottom: "1px solid #2d2d30" }}>
                  ...
                  <td style={{ padding: "12px", textAlign: "center" }}>
                    <button
                      type="button"
                      onClick={() => setSettleConfirmModal(p)}
      ...
      {/* النصف السفلي: إضافة شريك + تسجيل سحب مالي */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16 }}>
        ...
        <div style={styles.card}>
          ...
          <form onSubmit={handleAdd} style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 12 }}>
            ...
            <Field label="مبلغ الاستثمار (رأس المال)">
          ...
          <form onSubmit={handleWithdraw} style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 12 }}>
            <Field label="اسم الشريك">
              <select style={styles.input} value={withdrawPartner} onChange={e => setWithdrawPartner(e.target.value)} required>
                <option value="">اكتب أو اختر اسم الشريك...</option>
                {partners.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
              </select>
            </Field>

            <Field label="مبلغ السحب (ج.م) *">
              <input type="number" style={styles.input} value={withdrawAmount} onChange={e => setWithdrawAmount(e.target.value)} placeholder="0" required />
            </Field>

            <Field label="بيان وسبب السحب">
              <input style={styles.input} value={withdrawNotes} onChange={e => setWithdrawNotes(e.target.value)} placeholder="مثال: سحب نقدي تحت حساب الأرباح" />
            </Field>
      ...
      {/* نافذة ملف الشريك المنبثقة للضغط بالسجل وتفلترها */}
      ...
      {/* نافذة تأكيد تصفية حساب الشريك */}
      {settleConfirmModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
          <div style={{ ...styles.card, width: "100%", maxWidth: 440, textAlign: "center" }}>
            <h3 style={{ color: "#e07a5f", fontSize: 18, fontWeight: 800, marginBottom: 8 }}>
              تأكيد تصفية وحذف حساب الشريك: {settleConfirmModal.name}
            </h3>
            <p style={{ color: "#c4c4c4", fontSize: 13, marginBottom: 16, lineHeight: 1.6 }}>
              عند التأكيد، سيتم تصفية رصيد الشريك وتسوية حسابه بالكامل، وتخصيم كامل رأس ماله ({fmt(settleConfirmModal.capital)} ج.م) من رأس مال الشركة الإجمالي وتعديل نسب باقي الشركاء تلقائياً.
            </p>
            <div style={{ display: "flex", gap: 10 }}>
              <button
                type="button"
                onClick={() => {
                  onSettlePartner(settleConfirmModal.id);
                  setSettleConfirmModal(null);
                }}
                style={{ ...styles.saveBtn, flex: 1, background: "#e07a5f", color: "#fff", marginTop: 0 }}
              >
                تأكيد التصفية والحذف
              </button>
              <button
                type="button"
                onClick={() => setSettleConfirmModal(null)}
                style={{ background: "#1b1b1d", border: "1px solid #404040", color: "#fff", borderRadius: 12, padding: "12px 18px", cursor: "pointer", fontWeight: 700 }}
              >
                إلغاء
              </button>
            </div>
...
/* نافذة تفاصيل وسحوبات الشريك المنبثقة مع الفلترة */
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
      <div style={{ background: "#242426", border: "1px solid #d0b689", borderRadius: 18, width: "100%", maxWidth: 580, padding: 22, color: "#fff", position: "relative" }}>
        ...
        <button onClick={onClose} style={{ position: "absolute", top: 16, left: 16, background: "#1b1b1d", border: "1px solid #404040", color: "#e8cd9c", width: 34, height: 34, borderRadius: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <X size={18} />
        </button>

        <h3 style={{ fontSize: 18, fontWeight: 800, color: "#e8cd9c", marginBottom: 12 }}>
          سجل سحوبات الشريك: {partner.name}
        </h3>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, background: "#1b1b1d", padding: 12, borderRadius: 12, marginBottom: 16 }}>
          ...
          <div><span style={{ fontSize: 11, color: "#888" }}>نسبة الشراكة:</span> <div style={{ fontSize: 15, fontWeight: 800, color: "#d0b689" }}>{partner.sharePct}%</div></div>
          <div><span style={{ fontSize: 11, color: "#888" }}>إجمالي المسحوبات بالفلترة:</span> <div style={{ fontSize: 15, fontWeight: 800, color: "#f0c6bb" }}>{fmt(filteredTotal)} ج.م</div></div>
        </div>

        {/* فلترة تاريخية من و إلى */}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", color: "#fff", textAlign: "right", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#1b1b1d", color: "#c4c4c4", borderBottom: "1px solid #404040" }}>
                ...
                <th style={{ padding: "8px" }}>المبلغ المسحوب</th>
                <th style={{ padding: "8px" }}>البيان</th>
              </tr>
            </thead>
            <tbody>
              {partnerWithdrawals.length === 0 ? (
                <tr><td colSpan={3} style={{ padding: 16, textAlign: "center", color: "#888" }}>لا توجد مسحوبات مسجلة لهذا الشريك في الفترة المحددة.</td></tr>
              ) : (
...
/* ============================================================
   3. شاشة المصروفات المفلترة (مطابقة للصورة 4 مع التعديلات)
   ============================================================ */
        ...
        <form onSubmit={handleAdd} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14 }}>
          ...
          <Field label="البند">
            <select style={styles.input} value={category} onChange={e => setCategory(e.target.value)}>
              ...
              <option value="كهرباء ومياه وغاز">فواتير كهرباء</option>
          ...
          <div style={{ gridColumn: "1 / -1", marginTop: 6 }}>
            <button type="submit" style={styles.saveBtn}>تسجيل المصروف</button>
          </div>
        </form>
      </div>

      {/* سجل المصروفات مع فلترة من وإلى */}
      <div style={styles.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: "#e8cd9c" }}>
            سجل المصروفات (إجمالي: {fmt(totalFiltered)} ج.م)
          </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", color: "#fff", textAlign: "right", fontSize: 14 }}>
            ...
            <tbody>
              {filteredExpenses.length === 0 ? (
                <tr><td colSpan={5} style={{ padding: 20, textAlign: "center", color: "#888" }}>لا توجد مصروفات ينطبق عليها شرط البحث.</td></tr>
              ) : (
                filteredExpenses.slice().reverse().map((exp) => (
                  <tr key={exp.id} style={{ borderBottom: "1px solid #2d2d30" }}>
                    ...
                    <td style={{ padding: "10px", textAlign: "center" }}>
                      <button
                        type="button" onClick={() => handleDelete(exp.id)}
...
/* ============================================================
   4. شاشة إدارة الموظفين والرواتب والسلف المدمجة (تدمج 5 و 6)
   ============================================================ */
function EmployeesMergedScreen({ employees, setEmployees, salaryLog, setSalaryLog, onBack, notify }) {
  ...
  return (
    <div style={styles.container}>
      <ScreenHeader title="إضافة موظف جديد وقوائم الموظفين والرواتب" onBack={onBack} />

      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        <button
          type="button" onClick={() => setActiveTab("employees")}
          style={{
            flex: 1, padding: "12px", borderRadius: 12, border: "1px solid #404040",
            background: activeTab === "employees" ? "linear-gradient(145deg, #e8cd9c, #d0b689)" : "#1b1b1d",
            color: activeTab === "employees" ? "#1b1b1d" : "#c4c4c4", fontWeight: 800, fontSize: 14, cursor: "pointer"
          }}
        >
          👤 إضافة وقائمة الموظفين
        </button>
        <button
          type="button" onClick={() => setActiveTab("salaries")}
          style={{
            flex: 1, padding: "12px", borderRadius: 12, border: "1px solid #404040",
            background: activeTab === "salaries" ? "linear-gradient(145deg, #e8cd9c, #d0b689)" : "#1b1b1d",
            color: activeTab === "salaries" ? "#1b1b1d" : "#c4c4c4", fontWeight: 800, fontSize: 14, cursor: "pointer"
          }}
        >
          💵 رواتب وسلف الموظفين
        </button>
          ...
          <div style={styles.card}>
            ...
            <form onSubmit={handleAddEmp} style={{ ...styles.formGrid, marginTop: 12 }}>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                <Field label="تاريخ التعيين"><DateInput value={empForm.hireDate} onChange={e => setEmpForm({ ...empForm, hireDate: e.target.value })} required /></Field>
              </div>
                ...
                <tbody>
                  {employees.map(emp => (
                    <tr key={emp.id} style={{ borderBottom: "1px solid #2d2d30" }}>
                      <td
                        style={{ padding: "10px", fontWeight: 800, color: "#e8cd9c", cursor: "pointer", textDecoration: "underline" }}
                      ...
                      <td style={{ padding: "10px", color: emp.status === "نشط" ? "#bfe8cd" : "#e07a5f", fontWeight: 700 }}>
                        {emp.status || "نشط"}
                      </td>
                    </tr>
                  ))}
            ...
            <form onSubmit={handleTransSubmit} style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 12 }}>
              <Field label="اسم الموظف">
                <select style={styles.input} value={selectedEmpName} onChange={e => setSelectedEmpName(e.target.value)} required>
                  <option value="">اكتب اسم الموظف...</option>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
                ...
                <Field label="المبلغ (ج.م) *">
                  <input type="number" style={styles.input} value={transAmount} onChange={e => setAmount(e.target.value)} placeholder="0" required />
                </Field>
              </div>

              <Field label="ملاحظات الحركة">
                <input style={styles.input} value={transNotes} onChange={e => setNotes(e.target.value)} placeholder="تفاصيل..." />
              </Field>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", color: "#fff", textAlign: "right", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#1b1b1d", color: "#c4c4c4", borderBottom: "1px solid #404040" }}>
                    ...
                    <th style={{ padding: "8px" }}>نوع الحركة</th>
                ...
                <tbody>
                  {salaryLog.length === 0 ? (
                    <tr><td colSpan={5} style={{ padding: 16, textAlign: "center", color: "#888" }}>لا يوجد حركات مسجلة بعد.</td></tr>
                  ) : (
      ...
      {/* نافذة الموظف المنبثقة لسجل سحوباته وفسخ العقد */}
...
/* نافذة الموظف التفصيلية المنبثقة مع الفلترة وفسخ العقد */
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
      <div style={{ background: "#242426", border: "1px solid #d0b689", borderRadius: 18, width: "100%", maxWidth: 600, padding: 22, color: "#fff", position: "relative" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, background: "#1b1b1d", padding: 12, borderRadius: 12, marginBottom: 16 }}>
          ...
          <div><span style={{ fontSize: 11, color: "#888" }}>إجمالي المالي بالفلترة:</span> <div style={{ fontSize: 14, fontWeight: 800, color: "#f0c6bb" }}>{fmt(totalEmpPaid)} ج.م</div></div>
        </div>
        ...
        <div style={{ overflowX: "auto", maxHeight: 200, overflowY: "auto", marginBottom: 16 }}>
          <table style={{ width: "100%", borderCollapse: "collapse", color: "#fff", textAlign: "right", fontSize: 13 }}>
            ...
            <tbody>
              {empLogs.length === 0 ? (
                <tr><td colSpan={4} style={{ padding: 16, textAlign: "center", color: "#888" }}>لا توجد حركات مسجلة للموظف بالفلترة.</td></tr>
              ) : (
        ...
        {employee.status !== "فسخ عقد / منتهي" && (
          <button
            type="button" onClick={() => onTerminate(employee.id)}
            style={{ width: "100%", background: "#3a2320", border: "1px solid #7a4a3f", color: "#f0c6bb", padding: "10px", borderRadius: 10, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
          >
            <UserMinus size={16} /> فسخ العقد وإنهاء خدمة الموظف
          </button>
        )}
...
/* ============================================================
   5. شاشة توزيع الأرباح على الشركاء الفائقة (مطابقة للصورة 3)
   ============================================================ */
function ProfitDistributionScreen({ partners, setPartners, expenses, salaryLog, withdrawalsLog, setWithdrawalsLog, distributionsLog, setDistributionsLog, totals, onBack, notify }) {
  ...
  // حساب أرباح، مصروفات، ورواتب الفترة المحددة
  ...
  const partnersCalculated = useMemo(() => {
    return partners.map((p) => {
      ...
      return {
        ...
        prevWithdrawals: partnerWithdrawals,
        shareAmount: amountForThisPartner,
        decision
      };
    });
  ...
  const toggleDecision = (partnerId) => {
    setPartnerDecisions(prev => ({
      ...prev,
      [partnerId]: prev[partnerId] === "إعادة استثمار" ? "سحب فوري" : "إعادة استثمار"
    }));
  };
  ...
  const handleExecuteDistribution = () => {
    ...
    partnersCalculated.forEach((p) => {
      if (p.decision === "إعادة استثمار") {
        ...
        const newW = {
          id: Date.now() + Math.random(), partnerId: p.id, partnerName: p.name,
          amount: p.shareAmount, date: new Date().toISOString().split("T")[0], notes: "توزيع أرباح - سحب فوري"
        };
        setWithdrawalsLog(prev => [...prev, newW]);
      }
    });
    ...
    const newDist = {
      ...
      amount: autoAmount, details: partnersCalculated.map(p => `${p.name}: ${fmt(p.shareAmount)} ج.م (${p.decision})`).join(" | ")
    };
    ...
    <div style={styles.container}>
      ...
      <div style={{ ...styles.card, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, background: "#1b1b1d", border: "1px dashed #d0b689", borderRadius: 12, padding: 12, textAlign: "center" }}>
          <div><div style={{ fontSize: 11, color: "#888" }}>المصروفات في الفترة</div><div style={{ fontSize: 15, fontWeight: 800, color: "#f0c6bb" }}>{fmt(filteredPeriodData.periodExpenses)} ج.م</div></div>
          <div><div style={{ fontSize: 11, color: "#888" }}>الرواتب والسلف في الفترة</div><div style={{ fontSize: 15, fontWeight: 800, color: "#f0c6bb" }}>{fmt(filteredPeriodData.periodSalaries)} ج.م</div></div>
          <div><div style={{ fontSize: 11, color: "#888" }}>الصافي القابل للتوزيع</div><div style={{ fontSize: 16, fontWeight: 800, color: "#e8cd9c" }}>{fmt(filteredPeriodData.netPeriodProfit)} ج.م</div></div>
        </div>
      </div>

      {/* المبلغ المراد توزيعه فعلياً */}
      <div style={{ ...styles.card, marginBottom: 20 }}>
      ...
      {/* جدول توزيع المبلغ بقرارات الشركاء */}
      <div style={{ ...styles.card, marginBottom: 20 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: "#e8cd9c", marginBottom: 12 }}>
          توزيع المبلغ حسب نسبة كل شريك (اضغط اسم الشريك لاستعراض حركاته)
        </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", color: "#fff", textAlign: "right", fontSize: 14 }}>
            <thead>
            ...
            <tbody>
              {partnersCalculated.map(p => (
                <tr key={p.id} style={{ borderBottom: "1px solid #2d2d30" }}>
                  ...
                  <td style={{ padding: "10px", textAlign: "center" }}>
                    <button
                      type="button" onClick={() => toggleDecision(p.id)}
                      style={{
                        background: p.decision === "إعادة استثمار" ? "#213526" : "#211f18",
                        border: `1px solid ${p.decision === "إعادة استثمار" ? "#3d6b4a" : "#d0b689"}`,
                        color: p.decision === "إعادة استثمار" ? "#bfe8cd" : "#e8cd9c",
                        padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 800, cursor: "pointer",
                        display: "inline-flex", alignItems: "center", gap: 4
                      }}
                    >
                      {p.decision === "إعادة استثمار" ? <RefreshCw size={12} /> : <DollarSign size={12} />}
                      {p.decision}
                    </button>
                  </td>
                </tr>
        ...
        <button type="button" onClick={handleExecuteDistribution} style={{ ...styles.saveBtn, marginTop: 16 }}>
          تنفيذ التوزيع على كل الشركاء
        </button>
      </div>
      ...
      {distributionsLog.length > 0 && (
        <div style={styles.card}>
          <div style={{ fontSize: 15, fontWeight: 800, color: "#e8cd9c", marginBottom: 10 }}>سجل التوزيعات السابقة</div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", color: "#fff", textAlign: "right", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#1b1b1d", color: "#c4c4c4", borderBottom: "1px solid #404040" }}>
                  <th style={{ padding: "8px" }}>التاريخ</th>
                  <th style={{ padding: "8px" }}>إجمالي المبلغ</th>
                  <th style={{ padding: "8px" }}>البيان والتفاصيل</th>
                </tr>
              </thead>
              <tbody>
                {distributionsLog.slice().reverse().map(d => (
                  <tr key={d.id} style={{ borderBottom: "1px solid #2d2d30" }}>
                    <td style={{ padding: "8px" }}>{d.date}</td>
                    <td style={{ padding: "8px", color: "#e8cd9c", fontWeight: 800 }}>{fmt(d.amount)} ج.م</td>
                    <td style={{ padding: "8px", color: "#888" }}>{d.details}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
/* ============================================================
   باقي الشاشات والتنسيقات المساعدة
   ============================================================ */
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
      <div style={{ background: "#242426", border: "1px solid #d0b689", borderRadius: 18, width: "100%", maxWidth: 520, padding: 24, color: "#fff", position: "relative" }}>
        ...
        <button type="button" onClick={handlePrint} style={{ width: "100%", background: "linear-gradient(145deg, #e8cd9c, #d0b689)", color: "#1b1b1d", border: "none", borderRadius: 10, padding: "11px", fontWeight: 800, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 10 }}>
