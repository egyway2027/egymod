        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "14px", marginBottom: "14px" }}>
            <label style={labelStyle}>
              ...
              <input type={form.contractDate || contractFocused ? "date" : "text"} style={{ ...inputStyle, textAlign: "center", colorScheme: themeStyles?.isLight ? "light" : "dark" }} value={form.contractDate} onFocus={() => setContractFocused(true)} onBlur={(e) => { if (!e.target.value) setContractFocused(false); }} onChange={handleContractDate} placeholder={isEN ? "YYYY-MM-DD" : "سنة - شهر - يوم"} required />
            </label>
            <label style={labelStyle}>
              <span>{t.firstDueDateLabel || (isEN ? "First Due Date (Auto +1 Mo)" : "تاريخ أول قسط (تلقائي + شهر)")}</span>
              <input type={form.firstPayDate ? "date" : "text"} style={{ ...inputStyle, textAlign: "center", color: themeStyles.subText || "#888888", cursor: "not-allowed", colorScheme: themeStyles?.isLight ? "light" : "dark" }} value={form.firstPayDate} placeholder={isEN ? "YYYY-MM-DD" : "سنة - شهر - يوم"} disabled readOnly />
            </label>
          </div>
          <div>
            <label style={labelStyle}>
